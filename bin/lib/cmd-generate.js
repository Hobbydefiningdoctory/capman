...
module.exports = async function cmdGenerate() {
  ...
  if (fromFlag) {
    ...
    if (stats.skipped > 0) {
      log.warn(`Skipped ${stats.skipped} operations — could not determine endpoint shape`)
    }
    if (stats.autoSynthesized > 0) {
      log.info(
        `Auto-synthesized descriptions for ${stats.autoSynthesized} ` +
        `${stats.autoSynthesized === 1 ? 'capability' : 'capabilities'} ` +
        `(spec had no summary or description) — review these in ${configOut}`
      )
    }
